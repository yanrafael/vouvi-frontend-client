...
function UserReturn() {
  return (
    <section className="fixed left-0 top-0 z-20 flex h-[fill-available] w-full items-center justify-center overflow-y-hidden shadow-2xl backdrop-blur-[25px] dark:bg-black/50">
      <div className="flex h-[600px] w-8/12 flex-col items-center justify-center rounded-md bg-white p-6 shadow-2xl dark:bg-[#1B1B1B] dark:shadow-black">
        ...
        <div className="flex h-[400px] w-11/12 items-center justify-around rounded-md p-8">
          <Box
            icon={
              <Icon
...
function Box({ icon, cor, texto }) {
  return (
    <div
      className={`${cor ? "hover:bg-primary-200" : "hover:bg-secondary-200"} hover:rounded-md hover:border-8 hover:bg-opacity-30 ${cor ? "hover:border-[#7C20BE]" : "hover:border-secondary-200"} flex h-[250px] w-4/12 flex-col items-center justify-center rounded-md bg-secondary-200/20 p-4 text-black shadow-black dark:bg-black dark:text-white`}
    >
      {icon}
