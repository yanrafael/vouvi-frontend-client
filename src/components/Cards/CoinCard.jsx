function Card({ icon, title, text, color, background }) {
  return (
    ...
    >
      <img src={icon} alt="" />
      <h2 className="text-[17px] xs:text-[20px] sm:text-[28px] lg:text-[21px] xl:text-[26px] 2xl:text-[32px]">{title}</h2>
      <p className="text-[12px] xs:text-[15px] sm:text-[22px] lg:text-[16px] xl:text-[20px] 2xl:text-[24px]">{text}</p>
    </div>
  );
}
export default Card;
