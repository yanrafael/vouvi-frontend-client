...
function Top() {
    ...
    const activeButtonColor = 'bg-white text-black'
        ...
        <>
            <section className='w-11/12 m-auto flex justify-between bg-primary-100 outline outline-[26px] outline-[#7C20BE33] rounded-md px-10 pt-10 mt-24'>
                <div className='text-primary-200 w-3/6 mt-5 ml-5'>
                    ...
                    <p className='text-lg font-light mt-6'>Energia infinita, ferramentas avançadas e relatórios em PDF para toda a família!</p>
            ...
            <section className='m-auto w-11/12'>
                <div className='flex justify-between items-center mb-6'>
                    ...
                    <div onClick={() => activeButton(0)}>
                        <StoreButton color={`${btColor === 0 ? activeButtonColor : ''}`} text={'Todos'} >
                            <Icon icon={'solar:star-shine-bold'} />
                        </StoreButton>
                    </div>

                    <div onClick={() => activeButton(1)}>
                        <StoreButton color={`${btColor === 1 ? activeButtonColor : ''}`} text={'Olhos'}>
                            <Icon icon={'mingcute:eye-fill'} />
                        </StoreButton>
                    </div>

                    <div onClick={() => activeButton(2)}>
                        <StoreButton color={`${btColor === 2 ? activeButtonColor : ''}`} text={'Pelos'}>
                            <Icon icon={'icon-park-outline:texture'} />
                        </StoreButton>
                    </div>

                    <div onClick={() => activeButton(3)}>
                        <StoreButton color={`${btColor === 3 ? activeButtonColor : ''}`} text={'Coleiras'}>
                            <Icon icon={'carbon:circle-dash'} />
                        </StoreButton>
                    </div>

                    <div onClick={() => activeButton(4)}>
                        <StoreButton color={`${btColor === 4 ? activeButtonColor : ''}`} text={'Pingente'}>
                            <Icon icon={'mdi:necklace'} />
                        </StoreButton>
                    </div>

                    <div onClick={() => activeButton(5)}>
                        <StoreButton color={`${btColor === 5 ? activeButtonColor : ''}`} text={'Fundos'}>
